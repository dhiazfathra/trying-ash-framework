defmodule FnbErp.Fixtures do
  ...
  def location(attrs \\ %{}) do
    Warehouse.create_location!(Enum.into(attrs, %{name: uniq("Warehouse"), code: uniq("WH")}))
  end
  ...
  def product(attrs \\ %{}) do
    Catalog.create_product!(
      Enum.into(attrs, %{
        name: uniq("Cold Brew"),
        sku: uniq("SKU"),
        unit_price: Decimal.new("25000.00")
      })
    )
  end
  ...
  def order(customer, location, attrs \\ %{}) do
    Sales.create_order!(Enum.into(attrs, %{customer_id: customer.id, location_id: location.id}))
  end
  ...
  def order_with_line(opts \\ []) do
    ...
    line =
      Sales.add_order_line!(%{order_id: order.id, product_id: product.id, quantity: quantity})
  ...
  @doc "Ledger rows for a (product, location) pair, oldest first."
  def movements(product, location) do
    inventory =
      FnbErp.Warehouse.Inventory
      |> Ash.Query.filter(product_id == ^product.id and location_id == ^location.id)
      |> Ash.read_one!()
